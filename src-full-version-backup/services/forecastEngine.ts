...
export function computeSynapseForecast(
  ...
  for (let i = 0; i < hoursToProcess; i++) {
    ...
    const ghiVal = weather.global_horizontal_irradiance[i] ?? 0;
    const dniVal = weather.direct_normal_irradiance[i] ?? 0;
    const dhiVal = weather.diffuse_horizontal_irradiance[i] ?? 0;

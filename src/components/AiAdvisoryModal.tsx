...
export const AiAdvisoryModal: React.FC<AiAdvisoryModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-md animate-in fade-in duration-150">
      <div className={`rounded-2xl border shadow-2xl max-w-2xl w-full flex flex-col h-[600px] max-h-[90vh] transition-all ${
        isDarkMode
          ? 'bg-slate-900 border-slate-700/80 text-white'
          : 'bg-white border-slate-200 text-slate-900'
      }`}>
        {/* Modal Header */}
        <div className="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between bg-gradient-to-r from-slate-950 via-slate-900 to-indigo-950/50 text-white rounded-t-2xl">
        ...
        <div className={`flex-1 p-4 overflow-y-auto space-y-3 ${
          ...
          {messages.map((msg, index) => {
            ...
            return (
              ...
              >
                {isAi && (
                  <div className="w-7 h-7 rounded-lg bg-emerald-600 text-white flex items-center justify-center shrink-0 mt-0.5 text-xs shadow-md shadow-emerald-600/30">
                ...
                <div
                  className={`max-w-[85%] p-3.5 rounded-xl text-xs sm:text-sm leading-relaxed ${
                    isAi
                      ? isDarkMode
                        ? 'bg-slate-900 border border-slate-800 text-slate-200 shadow-xs whitespace-pre-line'
                        : 'bg-white border border-slate-200 text-slate-800 shadow-xs whitespace-pre-line'
                      : 'bg-emerald-600 text-white shadow-xs'
                  }`}
                >
                  <p>{msg.text}</p>
                  <span className={`block text-[10px] mt-1.5 ${isAi ? 'text-slate-500' : 'text-emerald-100'}`}>
                    {msg.time}
                  </span>
                </div>
                {!isAi && (
                  <div className="w-7 h-7 rounded-lg bg-slate-800 text-slate-300 flex items-center justify-center shrink-0 mt-0.5 text-xs border border-slate-700">
          ...
          {isLoading && (
            <div className="flex gap-2.5 items-center text-xs text-emerald-400 italic p-2">
        ...
        {/* Prompt Suggestions */}
        <div className={`px-4 py-2.5 border-t flex flex-wrap items-center gap-1.5 ${
          isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'
        }`}>
          <span className={`text-[11px] font-medium ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Ask:</span>
          {[
        ...
        {/* Input Form */}
        <form onSubmit={handleSendMessage} className={`p-3 border-t rounded-b-2xl flex gap-2 ${
          ...
          <input
            ...
            className={`flex-1 text-xs sm:text-sm px-3.5 py-2.5 rounded-xl border focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
              isDarkMode
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-bold bg-emerald-500 hover:bg-emerald-400 disabled:opacity-50 text-slate-950 flex items-center gap-1.5 transition-all shadow-md shadow-emerald-500/20 cursor-pointer"
          >
